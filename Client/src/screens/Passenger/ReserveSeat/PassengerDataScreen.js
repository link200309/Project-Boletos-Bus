...
export default function PassengerDataScreen({ navigation, route }) {
  const { selectedSeats = ['A01', 'A02'], travelDetails = { route: 'Cochabamba → La Paz', date: 'Viernes 20 de mayo', time: '07:30 → 13:00' } } = route.params || {};

  const [passengers, setPassengers] = useState(
    selectedSeats.map((seat) => ({ seat, firstName: '', lastName: '', identityNumber: '', birthDate: '' }))
  );
    ...
    <GenericContainer style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} horizontal>
        <View style={styles.contentContainer}>
          ...
          <ContactCard contact={contact} setContact={setContact} containerStyle={styles.card} />
